...
export const CategoryService = {
    getAll: (data: getPayLoad)=> api.get(`/task-category/all?${data.currentPage && `page=${data.currentPage}`}&${data.pageLimit && `limit=${data.pageLimit}`} ${data.search && `&search=${data.search}`}`),
