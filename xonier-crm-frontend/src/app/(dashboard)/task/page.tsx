  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2.5">
        <span
          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${pri.cls}`}
        >
      ...
      <div className="flex items-center justify-between pt-2.5 border-t border-gray-50 dark:border-gray-700 mt-1">
        ...
        {task.dueDate && (
          <span
            className={`text-[10px] font-semibold flex items-center gap-0.5 ${isOverdue ? "text-rose-500" : "text-gray-400 dark:text-gray-500"}`}
          >
            {isOverdue && "⚠️"}
          ...
          <div key={gi} className="animate-pulse">
            ...
            <div className="flex gap-4">
              {Array.from({ length: 3 }).map((_, ci) => (
                <div
                  key={ci}
                  className="min-w-[260px] bg-gray-50 dark:bg-gray-800/50 rounded-2xl border-2 border-gray-100 dark:border-gray-700 p-3 space-y-3"
                >
                  <div className="h-4 w-20 bg-gray-200 dark:bg-gray-700 rounded" />
                  {Array.from({ length: 2 }).map((_, ti) => (
                    <div
                      key={ti}
                      className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-3 space-y-2"
                    >
  ...
  const fetchTaskAll = async () => {
    try {
      const res = await TaskService.getAll({
        ...
        category:
          filtrCategory.length > 0 ? filtrCategory.join(",") : undefined,
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-1 mb-5">
          ...
          <CategoryMultiSelect
            ...
            selected={filtrCategory}
            onChange={(val) => {
              setFiltrCategory(val);
              setCurrentPage(1);
            }}
          />
          <UserSelect
                ...
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700">
                    {[
                      ...
                      ...(showActions
                        ? [{ label: "Actions", cls: "w-28 text-right" }]
                        : []),
                    ].map((col) => (
                      <th
                        key={col.label}
                    ...
                    taskData.map((task, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <StatusBadge
                              color={getColorOption(task.status.color)}
                          ...
                          <td className="px-5 py-4">
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${pri.cls}`}
                            >
                              <span
                          ...
                          <td className="px-5 py-4">
                            {task.dueDate ? (
                              <span
                                className={`text-xs font-semibold ${isOverdue ? "text-rose-500" : "text-gray-600 dark:text-gray-300"}`}
                              >
                              ...
                              <div className="flex items-center justify-end gap-2">
                                ...
                                {canView && (
                                  <button
                                    ...
                                    onClick={() =>
                                      router.push(
                                        `/task/view/${task.id}?userid=${task.assignedTo[0]?.id}`,
                                      )
                                    }
                                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-green-600 bg-green-50 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-400 transition"
                                  >
                                ...
                                {canEdit && (
                                  <button
                                    type="button"
                                    onClick={() =>
                                      router.push(`/task/update/${task.id}`)
                                    }
