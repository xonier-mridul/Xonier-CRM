...
const page = (): JSX.Element => {
  ...
  const getUserData = async (): Promise<void> => {
    try {
      ...
      // const result = await EnquiryService.getAllActiveWithoutPagination(); // adjust this call if needed
  ...
  const getEnquiryData = async () => {
    ...
    try {
      const result = await EnquiryService.getAll({
        ...
        // source: sourceFilter || undefined, // ✅ added sourceFilter
      });
  ...
  // Client-side filter: Sales Person filter compares against enquiry's createdBy user
  const filteredEnquiryData = React.useMemo(() => {
    if (!enquiryData || !Array.isArray(enquiryData)) return enquiryData;
    if (!salesPersonFilter) return enquiryData;
    return (enquiryData as any[]).filter((item) => item.createdBy?.id === salesPersonFilter);
  }, [enquiryData, salesPersonFilter]);
        ...
        <div className="flex items-center gap-10 justify-between">
          ...
          <div className=" items-center hidden lg:flex gap-6">
            ...
            <div className="bg-slate-50 text-slate-500 col-span-2 dark:bg-gray-600 px-3 py-2.5 rounded-lg border-[1px] border-slate-900/10 flex items-center gap-2">
              ...
              <input
                type="text"
                className="outline-none dark:text-white/70"
            ...
            <tbody>
              {!isLoading ? (
                filteredEnquiryData && Array.isArray(filteredEnquiryData) && filteredEnquiryData.length > 0 ? (
                  (filteredEnquiryData as any[]).map((item, i) => {
                    let rr = i % 2 == 0;
                    return (
                      <tr
                        key={item.enquiry_id}
                      ...
                      >
                        ...
                        <td className="p-4 ">
                          <span className="px-3 py-1 rounded-full bg-green-100 text-green-500 text-sm">{item.projectType}</span>
                        </td>
                        <td className="p-4 ">
                          <span className="bg-yellow-400 text-white px-3 py-1.5 text-sm rounded-sm">{item.source}</span>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-slate-500 text-[16px]  capitalize dark:text-white/70">
