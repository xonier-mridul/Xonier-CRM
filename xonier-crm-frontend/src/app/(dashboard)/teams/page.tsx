...
import { Team, TeamCategory, TeamCreatePayload } from "@/src/types/team/team.types";
...
const page = (): JSX.Element => {
  ...
  const getTeamData = async()=>{
    setIsLoading(true)
    try {
      const result = await TeamService.getAll({page: currentPage, limit: pageLimit})
  ...
  const handleDelete = async(id: string, name: string)=>{
    try {
      ...
      if(!id){
       ...
       const confirm =  await ConfirmPopup({title: "Are you sure", text: `Are you want to delete "${name}" team`, btnTxt: "Yes, delete"})
...
const result = await TeamService.delete(id)
  ...
  return (
    <>
    {isPopupShow && <> <BlurryBackground onClick={()=>setIsPopupShow(false)}/> <div
...
<div className="flex flex-col gap-2">
  ...
  <div className="flex justify-end gap-3 pt-4">
    ...
    <FormButton isLoading={isLoading} disabled={formData.name === "" || formData.members.length <= 0}> Submit </FormButton>
  </div>
</form>
            ...
            <tbody className="">
              {!isLoading ? (
                      ...
                      >
                        ...
                        <td className="first-letter:uppercase  ">
                          ...
                          {item?.members?.map((item, index)=>(
                            ...
                            <Link href={`/users/${item.id}`} key={index} className="bg-blue-100 hover:bg-blue-200 hover:scale-105 dark:bg-blue-200   text-blue-700 border border-blue-200 text-[13px] px-3 py-1 rounded-full transition-all duration"> {item.firstName} {item.lastName} </Link>
                            
                          ))}</div>
                        </td>
                        <td> <span className="bg-blue-500 text-white px-3 py-1 rounded-full text-[14px]"> {item.createdBy.firstName} {item.createdBy.lastName} </span></td>
                        <td>
                          <span
                            className={`${
                              item.isActive === true
                ...
                <Skeleton height={30} width={100} borderRadius={999} className="w-full "/>
                  ...
                  <td className="p-4">
                <Skeleton height={30} width={120} borderRadius={999} className="w-full "/>
                </td>
                  <td className="p-4">
                <Skeleton height={30} width={100} borderRadius={999} className="w-full "/>
                </td>
