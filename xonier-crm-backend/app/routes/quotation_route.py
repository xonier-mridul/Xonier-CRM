...
Depends(dependencies.company_context), Depends(dependencies.feature_access(FEATURE.CRM)), Depends(dependencies.permissions(["quote:create"]))])
...
@router.get("/get-all", status_code=200, dependencies=[Depends(dependencies.authorized),Depends(dependencies.company_active),
...
Depends(dependencies.company_context), Depends(dependencies.feature_access(FEATURE.CRM)), Depends(dependencies.permissions(["quote:update"]))])
...
@router.post("/resend/{id}", status_code=200, dependencies=[Depends(dependencies.authorized), Depends(dependencies.company_active),
Depends(dependencies.company_context), Depends(dependencies.feature_access(FEATURE.CRM)), Depends(dependencies.permissions(["quote:create"]))])
