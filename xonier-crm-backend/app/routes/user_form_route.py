...
@router.get("/user-id/lead", status_code=200, dependencies=[Depends(dependencies.authorized), Depends(dependencies.company_active),
Depends(dependencies.company_context), Depends(dependencies.permissions(["lead:create"]))])
async def find_by_user_id(request: Request):
    return await controller.get_lead_by_user_id(request)

@router.get("/user-id/deal", status_code=200, dependencies=[Depends(dependencies.authorized), Depends(dependencies.company_active),
Depends(dependencies.company_context), Depends(dependencies.permissions(["lead:create"]))])
async def find_by_user_id(request: Request):
